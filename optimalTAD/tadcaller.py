...
class armatus:
    def __init__(self, np, resolution, path_to_data, gamma_max, stepsize, output):
        self.np = str(np)
        self.resolution = str(resolution)
        self.input = path_to_data
        self.gamma_max = str(gamma_max)
        self.stepsize = str(stepsize)
        self.output = output + "armatus"
        self.path_to_armatus = './include/ArmatusParallel/src/armatus'
    
    def run(self):
        os.system("mpiexec -np {} {} -r {} -i {} -g {} -o {} -s {} -m".format(self.np,
...
def run_tadcaller(args, data, file):
    ...
    path_to_tads = check_path(path, "tads", filename.split(".")[0])
    ...
    for chromosome in data.keys():
        ...
        caller = armatus(args.np, args.resolution, path_to_file, args.gamma_max, args.stepsize, path_to_tads)
        caller.run()
